"""
scripts/actualizar_tc.py
Tipo de Cambio diario — ARS/USD
================================
Fuentes en cascada (la primera que responde gana):
  1. API BCRA v2  — tipo de cambio minorista de referencia (variable 4)
  2. bluelytics   — BNA oficial vendedor
  3. dolarapi.com — BNA oficial
  4. Fallback     — último valor guardado en tc.json, o hardcoded
...
Uso desde otros scripts:
  ...
  tc = cargar_tc()  # devuelve float (oficial_venta)
"""
...
# ── Config ────────────────────────────────────────────────────────────────────
TC_JSON = "tc.json"
TC_HARDCODED = 1420.0      # BNA venta 19/03/2026 — actualizar si pasa más de 1 semana
...
def _desde_bcra() -> dict | None:
    """BCRA API v2 — variable 4 = TC minorista referencia BNA."""
    try:
        r = requests.get(
...
def _desde_bluelytics() -> dict | None:
    """bluelytics.com.ar — BNA oficial + blue."""
    try:
        r = requests.get(
        ...
        if venta > 500:
            return {
                "oficial_venta": venta,
                "oficial_compra": float(oficial.get("value_buy", venta * 0.965)),
...
def _desde_dolarapi() -> dict | None:
    ...
    try:
        ...
        if venta > 500:
            return {
                "oficial_venta": venta,
                "oficial_compra": float(data.get("compra", venta * 0.965)),
                "blue_venta": None,
                "fuente": "dolarapi.com",
            }
    except Exception as e:
...
def _desde_argentinadatos() -> dict | None:
    """ArgentinaDatos.com — BNA oficial + blue."""
    try:
        r = requests.get(
...
def cargar_tc(campo: str = "oficial_venta") -> float:
    """
    Carga el TC desde tc.json. Si no existe o es muy viejo, intenta actualizar.
    Uso desde otros scripts:
        from scripts.actualizar_tc import cargar_tc
        tc = cargar_tc()
    """
    # Buscar tc.json en raíz o directorio actual
        ...
        if os.path.exists(path):
            try:
                ...
                if val > 500:
                    ...
                    if fecha == hoy:
                        return val
                    # Viejo pero usable como fallback
                    print(f"  ℹ️  TC de {fecha} (no es hoy). Usando igual: {val}")
                    return val
            except Exception:
                pass
    # No hay JSON → usar hardcoded
    print(f"  ⚠️  tc.json no encontrado. Usando fallback: {TC_HARDCODED}")
    return TC_HARDCODED


def guardar_tc(data: dict) -> None:
    """Guarda el TC en tc.json en la raíz del proyecto."""
    now = datetime.now()
    data["fecha"] = now.strftime("%Y-%m-%d")
    data["hora"]  = now.strftime("%H:%M")

    # Intentar guardar en raíz del proyecto (dos niveles arriba de scripts/)
    paths_destino = [
        TC_JSON,
...
def main() -> dict:
    print("=" * 50)
    print("=== MEL-TP: Actualizando Tipo de Cambio ===")
    print("=" * 50)
    ...
    if not resultado:
        # Intentar cargar último valor guardado
        paths = [TC_JSON, os.path.join(os.path.dirname(__file__), "..", TC_JSON)]
        for path in paths:
            if os.path.exists(path):
                try:
                    ...
                    if float(old.get("oficial_venta", 0)) > 500:
                        ...
                        print(f"      Usando último valor guardado ({old.get('fecha')}): ${old['oficial_venta']:,.2f}")
                        resultado = old
